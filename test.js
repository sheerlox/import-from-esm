...
const testImportFrom = async (t, dir, file, ext) => {
	...
	t.is((await importFrom(dir, moduleId)).default, 'unicorn');

	const moduleNotFoundError = await t.throwsAsync(
		async () => (await importFrom(dir, nonExistentModuleId))?.default
	);
...
test('importFrom.silent() (CJS)', async t => {
	t.is((await importFrom.silent('fixture/module', './fixture-cjs.cjs')).default, 'unicorn');
	t.is((await importFrom.silent('fixture/module', './nonexistent'))?.default, undefined);
});

test('importFrom.silent() (ESM)', async t => {
	t.is((await importFrom.silent('fixture/module', './fixture-esm.js')).default, 'unicorn');
	t.is((await importFrom.silent('fixture/module', './nonexistent'))?.default, undefined);
});
